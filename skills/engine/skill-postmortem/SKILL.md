...
description: Use after significant test failures, build breaks, wrong output, or user corrections when a per-skill post-mortem is needed. Auto-documents significant skill failures with root cause analysis, proposes patches to the involved skill, updates LESSONS_LEARNED.md in its directory. Complements skill-retrospective (session-wide) with per-skill analysis.
type: discipline
---
