...
Step 5b: User data
  → If .leopoldo/ contains user data:
    → "Delete Leopoldo data directory too? [y/N]"
...
| `session-start.sh` | SessionStart | Initialize session, check evolution |
| `compact-reinject.sh` | SessionStart (compact) | Re-inject gate state after context compaction |
...
6. Remove `.leopoldo/` directory (after confirming with user)
...
`.leopoldo/` is a protected directory containing user data. No manager operation touches it.

### Uninstall Confirmation

When running `/leopoldo uninstall`, after the standard confirmation:
1. Check if `.leopoldo/` contains user data
2. If yes, ask: "Delete Leopoldo data directory too? [y/N]"
