...
### set_config() + Neon HTTP adapter (INCOMPATIBILE)

**MAI usare `set_config('app.current_user_id', ..., true)` con il Neon HTTP adapter (`neon()` da `@neondatabase/serverless`).**

Ogni statement HTTP e' una connessione separata — il parametro `true` (transactional) non ha effetto perche' non esiste un contesto transazionale condiviso. RLS policies che usano `current_setting()` non funzionano.

**Soluzioni:**
- `sql.transaction()` per wrappare set_config + query nella stessa transazione
- WebSocket Pool (`neonConfig.webSocketConstructor`) per connessioni persistenti
- Application-level enforcement (DAL middleware) — raccomandato per <50 utenti

> Questa e' una limitazione nota del Neon HTTP adapter. Verificare sempre il metodo di connessione prima di usare set_config().
