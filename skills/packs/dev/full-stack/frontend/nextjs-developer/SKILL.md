...
|-------|-----------|-----------|
| App Router Patterns | `references/app-router-patterns.md` | File-based routing, layouts, templates, route groups, migration from Pages Router, metadata, generateStaticParams |
| Server vs Client Components | `references/server-client-components.md` | RSC patterns, 'use client' boundaries, cookies/headers access, searchParams, useSearchParams + Suspense, React 'use' API, streaming |
| Best Practices Index | `references/best-practices.md` | File conventions, RSC boundaries, async patterns, directives, error handling, data patterns, route handlers, metadata, image/font optimization, bundling, hydration errors, suspense, parallel routes, self-hosting, debug tricks |
...
This skill incorporates content previously maintained as separate skills:

- **App Router Patterns** (`references/app-router-patterns.md`) — from `nextjs-app-router-fundamentals` (wsimmonds/claude-nextjs-skills, MIT)
- **Server vs Client Components** (`references/server-client-components.md`) — from `nextjs-server-client-components` (wsimmonds/claude-nextjs-skills, MIT)
- **Best Practices** (`references/best-practices.md` + `references/best-practices/`) — from `next-best-practices` (vercel-labs/next-skills, no-license)
