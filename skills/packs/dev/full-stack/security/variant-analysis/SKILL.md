...
category: security
triggers:
  - pattern: "find variants"
  - pattern: "hunt bug variants"
  - pattern: "variant analysis"
  - pattern: "similar vulnerabilities"
  - pattern: "systematic code audit"
dependencies:
  hard: []
  soft:
    - codeql
...
- **What is the root cause?** Not the symptom, but WHY it is vulnerable
...
For deeper strategic guidance, see [references/methodology.md](references/methodology.md).
...
4. **Know when to stop**: 50%+ FP rate means you have gone too generic
...
**CodeQL** (`references/codeql/`):
- `python.ql`, `javascript.ql`, `java.ql`, `go.ql`, `cpp.ql`

**Semgrep** (`references/semgrep/`):
