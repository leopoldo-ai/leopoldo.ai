...
category: security
triggers:
  - pattern: "create semgrep rule"
  - pattern: "write semgrep rule"
  - pattern: "custom semgrep detection"
  - pattern: "semgrep rule for"
  - pattern: "build static analysis rule"
dependencies:
  hard: []
  soft:
    - semgrep
...
- Running existing Semgrep rulesets (use `semgrep` skill)
...
- **"It matches the vulnerable case"** - Matching vulnerabilities is half the job. Verify safe cases do not match (false positives break trust).
...
- **One YAML file - one Semgrep rule**: Each YAML file must contain only one Semgrep rule
- **No generic rules**: When targeting a specific language, avoid `languages: generic`
- **Forbidden `todook` and `todoruleid` test annotations**: These are not acceptable
...
**Iterating between approaches:** It is acceptable to experiment. If you start with taint mode and it does not work well (e.g., taint does not propagate as expected), switch to pattern matching. Conversely, if pattern matching produces too many false positives, try taint mode instead. The goal is a working rule.
...
- For detailed workflow and examples, you MUST see [references/workflow.md](references/workflow.md)
...
**REQUIRED**: Before writing any rule, use WebFetch to read **all** of these links with Semgrep documentation:
