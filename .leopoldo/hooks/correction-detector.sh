...
if echo "$PROMPT" | grep -iEq "$IT_PATTERN" 2>/dev/null || \
  ...
  cat <<'EOF'
{"additionalContext": "Note: this message may contain a correction of a previous output. If it refers to something Leopoldo just produced, consider running /postmortem before fixing."}
EOF
fi
