...
# --- Pack Awareness (dynamic routing support) ---
# Read installed packs from manifest and inject into context so the
# orchestrator can scope routing to skills that actually exist in this
# client's setup. Skill-router does runtime filesystem verification as backup.

MANIFEST="$ROOT/.leopoldo-manifest.json"

if [ -f "$MANIFEST" ] && _has_jq; then
  INSTALLED_PACKS="$(jq -r '(.plugins // {}) | to_entries | map(.key) | join(", ")' "$MANIFEST" 2>/dev/null || echo "")"
  if [ -n "$INSTALLED_PACKS" ] && [ "$INSTALLED_PACKS" != "null" ]; then
    CONTEXT="$CONTEXT INSTALLED_PACKS: $INSTALLED_PACKS."
  fi
fi

# --- System State (WS-C.1) ---
# Inline .leopoldo/system.md into session context so Claude knows the
# project's direction, personas, patterns, and decisions. Optional file;
# silently skipped if missing. Hook never writes (file is user-owned, edits
# go through orchestrator confirmation loop). See
# docs/guides/leopoldo-state-convention.md.

SYSTEM_MD="$ROOT/.leopoldo/system.md"
if [ -f "$SYSTEM_MD" ]; then
  SM_BODY="$(cat "$SYSTEM_MD")"
  if [ -n "$SM_BODY" ]; then
    CONTEXT="$CONTEXT"$'\n'"=== SYSTEM STATE ==="$'\n'"$SM_BODY"$'\n'"=== END SYSTEM STATE ==="
  fi
fi

# --- Active Playbooks (WS-C2) ---
# Scan .leopoldo/playbooks/*.yaml for files with `active: true` and inline
# their content into the session context. Clients use playbooks to encode
# their commercial standard (IC memo conventions, contract review checklist,
# etc). Skills read ACTIVE_PLAYBOOKS and opt in. See
# docs/guides/leopoldo-playbook-convention.md for schema.
# Uses grep/awk only (no yq). Fails silently if dir missing.

PLAYBOOKS_DIR="$ROOT/.leopoldo/playbooks"
if [ -d "$PLAYBOOKS_DIR" ]; then
  ACTIVE_SLUGS=""
  for pbfile in "$PLAYBOOKS_DIR"/*.yaml; do
    [ -f "$pbfile" ] || continue
    # Detect `active: true` at column 0 (ignores indented/nested keys)
    if grep -Eq '^active:[[:space:]]*true[[:space:]]*$' "$pbfile" 2>/dev/null; then
      # grep may find no match on YAML without explicit slug; || true prevents
      # pipefail from tripping the ERR trap and skipping the basename fallback.
      slug="$(grep -E '^slug:' "$pbfile" 2>/dev/null | head -1 | awk '{print $2}' | tr -d '"')" || slug=""
      [ -z "$slug" ] && slug="$(basename "$pbfile" .yaml)"
      ACTIVE_SLUGS="${ACTIVE_SLUGS:+$ACTIVE_SLUGS, }$slug"
      PB_BODY="$(cat "$pbfile")"
      CONTEXT="$CONTEXT"$'\n'"=== PLAYBOOK: $slug ==="$'\n'"$PB_BODY"$'\n'"=== END PLAYBOOK ==="
    fi
  done
  if [ -n "$ACTIVE_SLUGS" ]; then
    CONTEXT="$CONTEXT ACTIVE_PLAYBOOKS: $ACTIVE_SLUGS."
  fi
fi
