...
# If checkpoint.completed was logged AFTER the last gate.activated,
# auto-clear the gate. Checks temporal ordering to avoid clearing
# based on stale checkpoint.completed events.
# ──────────────────────────────────────────────
if [[ "$TOOL_NAME" == "Bash" ]] && _has_jq && [[ -n "$GATE_STATE_RAW" ]]; then
  ...
  if [[ "$CP_STATUS" == "pending" ]]; then
    ...
    if [[ -f "$JOURNAL_FILE" ]]; then
      CP_LAST_ACTIVATED="$(grep -n '"gate\.activated".*"checkpoint"' "$JOURNAL_FILE" 2>/dev/null | tail -1 | cut -d: -f1 || echo "0")"
      CP_LAST_COMPLETED="$(grep -n '"checkpoint\.completed"' "$JOURNAL_FILE" 2>/dev/null | tail -1 | cut -d: -f1 || echo "0")"
      if [[ "$CP_LAST_COMPLETED" -gt "$CP_LAST_ACTIVATED" ]] && [[ "$CP_LAST_COMPLETED" -gt 0 ]]; then
        update_gate_field '
          .gates.checkpoint.status = "clear"
          | .gates.checkpoint.soft_warnings = 0
          | .task_count_since_checkpoint = 0
        '
        journal_append "{\"event\":\"checkpoint.completed\",\"source\":\"auto_clear\",\"session_id\":\"$GATE_SESSION_ID\",\"timestamp\":\"$TIMESTAMP\"}"
        journal_append "{\"event\":\"gate.auto_cleared\",\"gate\":\"checkpoint\",\"reason\":\"checkpoint.completed detected in tool-logger (after gate.activated)\",\"session_id\":\"$GATE_SESSION_ID\",\"timestamp\":\"$TIMESTAMP\"}"
      fi
    fi
  fi
fi
...
if [[ "$TOOL_NAME" == "Edit" ]] || [[ "$TOOL_NAME" == "Write" ]] || [[ "$TOOL_NAME" == "Agent" ]]; then
  ...
  if [[ -f "$JOURNAL_FILE" ]]; then
    # Count Edit/Write/Agent events since last checkpoint reset point.
    # Reset points: checkpoint.completed, checkpoint.passed, OR gate.auto_cleared for checkpoint.
    # Using two greps + max to handle different JSON field orderings.
    CP_COMPLETED_LINE="$(grep -n '"checkpoint\.completed"\|"checkpoint\.passed"' "$JOURNAL_FILE" 2>/dev/null | tail -1 | cut -d: -f1 || echo "0")"
    CP_AUTOCLEARED_LINE="$(grep -n '"gate\.auto_cleared"' "$JOURNAL_FILE" 2>/dev/null | grep '"checkpoint"' | tail -1 | cut -d: -f1 || echo "0")"
    LAST_CP_LINE="$CP_COMPLETED_LINE"
    [[ "$CP_AUTOCLEARED_LINE" -gt "$LAST_CP_LINE" ]] && LAST_CP_LINE="$CP_AUTOCLEARED_LINE"
